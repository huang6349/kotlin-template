...
public interface TreeUtil {
    ...
    static String calculatePath(String parentPath, Long parentId) {
        if (StrUtil.isNotBlank(parentPath))
            return StrUtil.join(SEPARATOR, parentPath, parentId);
        else return ROOT;
    }

    static <T extends TreeEntity<T>> List<T> list2Tree(List<T> list, Long parentId) {
        ...
        list.forEach(treeEntity -> treeEntity.setChildren(groupMap.get(treeEntity.getId())));
